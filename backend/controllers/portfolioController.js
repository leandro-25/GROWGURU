...
class PortfolioController {
  static async getPortfolio(req, res) {
    try {
      ...
      logger.error('Erro ao carregar carteira:', error);
      res.status(500).json(ResponseFormatter.error('INTERNAL_ERROR', 'Erro ao carregar carteira'));
    }
  }
  ...
  static async buyAsset(req, res) {
    try {
      ...
      logger.error('Erro na compra:', error);
      res.status(500).json(ResponseFormatter.error('INTERNAL_ERROR', error.message || 'Erro ao processar compra'));
    }
  }
  ...
  static async sellAsset(req, res) {
    try {
      ...
      logger.error('Erro na venda:', error);
      res.status(500).json(ResponseFormatter.error('INTERNAL_ERROR', error.message || 'Erro ao processar venda'));
    }
  }
  ...
  static async getTotalInvested(req, res) {
    try {
      ...
      logger.error('Erro ao calcular total investido:', error);
      res.status(500).json(ResponseFormatter.error('INTERNAL_ERROR', 'Erro ao calcular total investido'));
    }
  }
  ...
  static async checkTableStructure(req, res) {
    try {
      ...
      res.json(ResponseFormatter.success({ message: 'Estrutura da tabela está correta', tableStructure: columns }));
    } catch (error) {
      logger.error('Erro ao verificar estrutura da tabela:', error);
      res.status(500).json(ResponseFormatter.error('INTERNAL_ERROR', 'Erro ao verificar estrutura da tabela'));
    }
  }
}
